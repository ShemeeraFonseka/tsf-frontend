...
const SeaFreightRatesForm = () => {
  ...
  const countryPortOptions = {
    ...
    "Sri Lanka": [{ code: "LKCMB", name: "Port of Colombo" }],
    UAE: [
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      parseFloat(form.rate_20ft) <= 0 ||
      parseFloat(form.rate_40ft) <= 0 ||
      parseFloat(form.kilos_20ft) <= 0 ||
      parseFloat(form.kilos_40ft) <= 0
    ) {
    ...
    try {
      ...
      const freight_per_kilo_20ft = calculateFreightPerKilo(
        form.rate_20ft,
        form.kilos_20ft,
      );
      const freight_per_kilo_40ft = calculateFreightPerKilo(
        form.rate_40ft,
        form.kilos_40ft,
      );
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${editingId ? "update" : "add"} sea freight rate`,
        );
      }
      ...
      if (data.recalculation) {
        ...
        setSuccess(
          `Sea freight rate ${editingId ? "updated" : "added"} successfully! ${data.recalculation.productsUpdated} product${data.recalculation.productsUpdated !== 1 ? "s" : ""} recalculated.`,
        );
      } else {
        setSuccess(
          `Sea freight rate ${editingId ? "updated" : "added"} successfully!`,
        );
      }
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm("Are you sure you want to delete this sea freight rate?")
    ) {
      return;
    }
  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit} className="apf-container">
        ...
        >
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            >
              ...
              >
                ${calculateFreightPerKilo(form.rate_20ft, form.kilos_20ft)}{" "}
                USD/kg
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            >
              ...
              >
                ${calculateFreightPerKilo(form.rate_40ft, form.kilos_40ft)}{" "}
                USD/kg
              </div>
            </div>
          </div>
        ...
        <label className="apf-label" style={{ marginTop: "20px" }}>
          Effective Date
        </label>
        <input
          className="apf-input"
      ...
      {recalcInfo && (
        ...
        >
          ...
          <div style={{ fontSize: "13px", color: "#424242" }}>
            ✓ Updated {recalcInfo.productsUpdated} product price
            {recalcInfo.productsUpdated !== 1 ? "s" : ""}
          </div>
          {recalcInfo.errors > 0 && (
            <div
              style={{ fontSize: "13px", color: "#d32f2f", marginTop: "3px" }}
            >
              ...
              <tbody>
                {getLatestRatesByCountryPort().map((rate) => (
                  <tr key={rate.id}>
                    ...
                    <td>${parseFloat(rate.rate_20ft).toFixed(2)}</td>
                    <td>{parseFloat(rate.kilos_20ft).toLocaleString()} kg</td>
                    <td>
                      ${parseFloat(rate.freight_per_kilo_20ft || 0).toFixed(4)}
                    </td>
                    <td>${parseFloat(rate.rate_40ft).toFixed(2)}</td>
                    <td>{parseFloat(rate.kilos_40ft).toLocaleString()} kg</td>
                    <td>
