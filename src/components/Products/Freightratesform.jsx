...
const FreightRatesForm = () => {
  ...
  const countryAirportOptions = {
    ...
    Sweden: [{ code: "ARN", name: "Stockholm Arlanda Airport" }],
    Norway: [{ code: "OSL", name: "Oslo Airport" }],
    Denmark: [{ code: "CPH", name: "Copenhagen Airport" }],
    Japan: [
    ...
    Qatar: [{ code: "DOH", name: "Hamad International Airport - Doha" }],
    China: [
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      parseFloat(form.rate_45kg) <= 0 ||
      parseFloat(form.rate_100kg) <= 0 ||
      parseFloat(form.rate_300kg) <= 0 ||
      parseFloat(form.rate_500kg) <= 0
    ) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${editingId ? "update" : "add"} freight rate`,
        );
      }
      ...
      if (data.recalculation) {
        ...
        setSuccess(
          `Freight rate ${editingId ? "updated" : "added"} successfully! ${data.recalculation.productsUpdated} product${data.recalculation.productsUpdated !== 1 ? "s" : ""} recalculated.`,
        );
      } else {
        setSuccess(
          `Freight rate ${editingId ? "updated" : "added"} successfully!`,
        );
      }
  ...
  const getLatestRatesByCountryAirport = () => {
    ...
    freightRates.forEach((rate) => {
      ...
      if (
        !ratesByCountryAirport[key] ||
        new Date(rate.updated_at) >
          new Date(ratesByCountryAirport[key].updated_at)
      ) {
                ...
                {getLatestRatesByCountryAirport().map((rate) => (
                  <tr key={rate.id}>
                    ...
                    <td>
                      ...
                      {rate.airport_name && <div>{rate.airport_name}</div>}
                    </td>
                    <td>${parseFloat(rate.rate_45kg).toFixed(2)}</td>
                    <td>${parseFloat(rate.rate_100kg).toFixed(2)}</td>
                    <td>${parseFloat(rate.rate_300kg).toFixed(2)}</td>
                    <td>${parseFloat(rate.rate_500kg).toFixed(2)}</td>
                    <td>
