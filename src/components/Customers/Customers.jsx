...
const Customers = () => {
  ...
  const handleDelete = async (e, cus_id, cus_name) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/customerlist/${cus_id}`, { method: 'DELETE' })
  ...
  const navigateForm       = ()           => navigate('/customerform')
  const navigateEdit       = (e, cus_id) => { e.stopPropagation(); navigate(`/customerform/${cus_id}`) }
  const navigateToDetail   = (cus_id)    => navigate(`/customer/${cus_id}`)
      ...
      {!loading && !error && (
        <div className="table-wrap">
          <table className="pricelist-table">
            ...
            <tbody>
              {customers.length === 0 && (
                <tr><td colSpan={8} className="muted">No customers found</td></tr>
              )}
