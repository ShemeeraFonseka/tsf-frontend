...
const UsdRateForm = () => {
  ...
  const fetchCurrentRate = async () => {
    try {
      ...
      console.error("Failed to fetch current rate:", e);
    }
  };
  ...
  const calculateExFactoryPrice = (
    ...
    const packingLKR =
      (parseFloat(packingCostUSD) || 0) * (parseFloat(usdRate) || 1);
    const labourLKR =
      (parseFloat(labourOverheadUSD) || 0) * (parseFloat(usdRate) || 1);
    return parseFloat(
      (purchase + packingLKR + labourLKR + (parseFloat(profit) || 0)).toFixed(
        2,
      ),
    );
  };
        ...
        if (product.variants?.length > 0) {
          try {
            const updatedVariants = product.variants.map((v) => ({
              ...
              exfactoryprice: calculateExFactoryPrice(
                v.purchasing_price,
                v.packing_cost,
                v.labour_overhead,
                v.profit,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const totalUpdated =
        (data.recalculation?.productsUpdated || 0) + exportResult.updated;
      const totalErrors =
        (data.recalculation?.errors || 0) + exportResult.errors;
      ...
      setSuccess(
        `USD rate updated successfully! ${totalUpdated} item${totalUpdated !== 1 ? "s" : ""} recalculated.`,
      );
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this rate entry?"))
      return;
    try {
      const res = await fetch(`${API_URL}/api/usd-rate/${id}`, {
        method: "DELETE",
      });
    ...
    <div className="usd-page">
        ...
        <div className="usd-current-rate">
          ...
          <div className="rate-meta">
            ...
            {currentRate.date && (
              <>
                <br />
                Rate date: {fmtDate(currentRate.date)}
              </>
            )}
          </div>
        </div>
      ...
      <div className="usd-warning">
        ...
        <ul>
          ...
          <li>
            Recalculates Ex-Factory prices (converts USD packing/overhead costs
            to LKR)
          </li>
      ...
      {recalcInfo && (
        <div className="recalc-panel">
          ...
          <div className="recalc-block">
            ...
            <p>
              ✓ Recalculated {recalcInfo.customerProductsUpdated} FOB &amp; CNF
              price{recalcInfo.customerProductsUpdated !== 1 ? "s" : ""}
            </p>
          </div>
          ...
          <div className="recalc-block">
            ...
            <p>
              ✓ Updated USD rate in {recalcInfo.exportProductsUpdated} product
              {recalcInfo.exportProductsUpdated !== 1 ? "s" : ""}
            </p>
            <p>✓ Recalculated Ex-Factory prices (USD costs → LKR)</p>
          </div>

          <div className="recalc-total">
            Total: {recalcInfo.totalUpdated} item
            {recalcInfo.totalUpdated !== 1 ? "s" : ""} updated
          </div>
        ...
        <div className="apf-success">
          ...
          {recalcInfo?.errors > 0 && (
            <div className="toast-warning">
              ⚠️ {recalcInfo.errors} item{recalcInfo.errors !== 1 ? "s" : ""}{" "}
              had errors
            </div>
          )}
        </div>
